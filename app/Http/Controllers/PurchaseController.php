...
public function index()
    {
        $purchases = Purchase::with('purchaseDetails.product')->orderBy('purchase_date', 'desc')->get();
   ...
   // crear una nueva compra con sus detalles
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'purchaseDetails.*.unit_price' => 'required|numeric|min:0|decimal:2',
        ]);
    ...
    {
        $purchase = Purchase::with('purchaseDetails.product', 'seller')->find($id);
...
// Devolver la compra con sus detalles
